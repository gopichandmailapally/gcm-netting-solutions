...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_bill'])) {
    ...
    $invoice_number = 'CASH-' . $company['invoice_prefix'] . str_pad($company['next_invoice_number'], 4, '0', STR_PAD_LEFT);
    
    $db->execute("INSERT INTO billing_invoices (invoice_number, invoice_type, customer_name, customer_phone, customer_address, invoice_date, subtotal, grand_total, notes, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [$invoice_number, 'cash_bill', $_POST['customer_name'], $_POST['customer_phone'], $_POST['customer_address'], $_POST['invoice_date'], $_POST['subtotal'], $_POST['grand_total'], $_POST['notes'], 'paid', $_POST['company_id']]
    );
    ...
    $db->execute("UPDATE billing_company_settings SET next_invoice_number = next_invoice_number + 1 WHERE id = ?", [$_POST['company_id']]);
    
    $_SESSION['success_message'] = "Cash bill created successfully!";
...
<div class="invoice-creator">
    <div class="page-header">
        <h1><i class="fas fa-money-bill-wave"></i> Create Cash Bill</h1>
    ...
    <form id="billForm" method="POST">
        ...
        <div class="content-card">
            ...
            <div class="card-body">
                ...
                <div id="companyPreview" style="display:none; margin: 20px 0; padding: 20px; background: linear-gradient(135deg, #FEF3C7, #FDE68A); border-radius: 12px; border: 2px solid #F59E0B;">
        ...
        <div class="content-card">
            <div class="card-body">
                <div class="form-group">
                    <label>Notes</label>
                    <textarea name="notes" class="form-control" rows="2" placeholder="Payment received in cash..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-success btn-large">
                <i class="fas fa-save"></i> Save Cash Bill
            </button>
...
function updateCompanyPreview() {
    ...
    if (select.value) {
        ...
        if (logo) {
            ...
            document.getElementById('companyLogoPreview').innerHTML = '<div style="width: 60px; height: 60px; background: linear-gradient(135deg, #F59E0B, #D97706); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; font-size: 24px;"><i class="fas fa-building"></i></div>';
        }
...
.totals-section { margin-top: 20px; padding: 20px; background: linear-gradient(135deg, #FEF3C7, #FDE68A); border-radius: 12px; }
...
.total-row.grand { border-top: 2px solid #F59E0B; padding-top: 12px; font-size: 20px; color: #1E293B; }
.amount-words { margin-top: 12px; padding: 12px; background: white; border-radius: 8px; font-weight: 600; color: #F59E0B; text-align: center; }
