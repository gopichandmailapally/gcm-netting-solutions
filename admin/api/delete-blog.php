...
if ($protection->isProtected('blog', $content_id)) {
    if (empty($pin)) {
        echo json_encode([
            'success'   => false,
            'protected' => true,
            'message'   => 'This blog is protected by AI Content Security. Enter your Security PIN to request deletion approval.',
        ]);
        exit;
    }
    $file_data = @json_decode(file_get_contents($file_path), true);
    $title     = $file_data['title'] ?? $content_id;
    $result    = $protection->requestDeleteWithApproval(
        'blog', $content_id, $title, $pin, $reason,
        $_SESSION['admin_id'] ?? 0
    );
    echo json_encode([
        'success'   => false,
